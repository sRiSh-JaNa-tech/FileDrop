...
async function setUser(name, userId) {
    const peerId = "p-" + Math.random().toString(36).substring(2, 8);
    try {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderRecentPeers = async () => {
        const { recentPeers } = await getStorage(['recentPeers']);
    ...
    const saveRecentPeer = async (peerName) => {
        ...
        await chrome.storage.local.set({ recentPeers: list });
        renderRecentPeers();
    };
    ...
    const sendHeartbeat = () => {
        const now = Date.now();
        if (now - lastHeartbeat > 2000) { // Throttle heartbeats to once every 2 seconds
    ...
    if (copyNameBtn) {
        copyNameBtn.addEventListener('click', () => {
            ...
            navigator.clipboard.writeText(name).then(() => {
                const originalText = copyNameBtn.innerHTML;
                ...
                setTimeout(() => {
                    copyNameBtn.innerHTML = originalText;
                    copyNameBtn.style.color = '';
                }, 2000);
            });
        });
    }
