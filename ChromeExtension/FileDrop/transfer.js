...
const sendHeartbeat = () => {
    const now = Date.now();
    if (now - lastHeartbeat > 2000) { // Once every 2 seconds max
...
async function transferFiles(file) {
    ...
    fileReader.onload = async (event) => {
        ...
        for (let i = 0; i < totalChunks; i++) {
            ...
            // Prevent flooding and send heartbeat
            if (i % 20 === 0) {
                sendHeartbeat();
...
function receiveFile(connection) {
    ...
    connection.on("data", (data) => {
        if (data.type === "identity") {
            ...
            receivedChunks++;
            if (receivedChunks % 50 === 0) sendHeartbeat(); // Periodic heartbeat
            ...
            if (receivedChunks < expectedChunks) {
                ...
                if (onReceiveProgress) onReceiveProgress({ type: 'error', message: 'Incomplete transfer' });
                return;
            }
